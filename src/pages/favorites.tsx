...
import { Course, SingleMenuItem, MenuItemInFavorites, MenuOfTheDay } from '../types/menu';
  ...
  function whenAvailable(mealString: string) {
    ...
    if(weeklyMenu.timeperiod.slice(3,5)+"-"+weeklyMenu.timeperiod.slice(0,2) == moment().startOf('isoWeek').format().slice(5,10)) {
      ...
      if(mealAvailableDate == moment().format('dddd')) {
        ...
        return <Typography sx={{color: "gold"}}>Available on {mealAvailableDate}</Typography>
      } else {
        return <Typography sx={{color: "red"}}>Not available this week</Typography>
      }
    } else {
